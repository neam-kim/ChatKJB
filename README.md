...
| `/synth <작업>` | 여러 제공자에게 읽기 전용 답변을 받은 뒤 상호 비판·보완·승점제 리그 심사·통합을 거칩니다. | 시간과 토큰을 많이 씁니다. |
...
| `src/judge.ts` | `/synth` 상호 비판, 보완, 승점제 리그 판정, 통합 보조 |
...
- 한도 회복 시 자동 재개가 예약된 뒤 원치 않으면 해당 세션 토픽에서 `/restop`으로 예약만 취소합니다.
