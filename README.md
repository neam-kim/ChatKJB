...
Telegram의 `/goal 조건`은 조건이 충족될 때까지 작업을 자동으로 이어 가게 한다(클로드 코드 본가의 `/goal`과 같은 개념). **Claude·Codex·agy 세 제공자 모두에서 동작한다** — 작업 턴은 그 세션의 제공자로 실행하고, 한 턴이 정상 종료될 때마다 작업 제공자와 무관하게 빠른 모델(Claude Haiku)로 "조건이 이미 충족됐는지"를 읽기 전용으로 판정한다(저장소 상태만 확인하므로 제공자 독립적이다). 미충족이면 사용자가 다시 시키지 않아도 같은 목표를 향한 다음 턴을 자동으로 예약한다(Codex는 직전 스레드, agy는 직전 대화를 재개). 충족되면 목표를 해제하고 알리며, 폭주 방지를 위해 최대 25턴까지만 자동 진행한다. `/goal`만 입력하면 현재 목표를, `/goal clear`는 목표를 해제한다. 유휴 상태에서 목표를 걸면 즉시 작업을 시작하고(이어 갈 세션이 한 번도 실행된 적 없으면 첫 메시지 이후부터 진행), 실행 중에 걸면 현재 턴이 끝난 뒤부터 평가한다. `/stop`이나 `/goal clear`로 언제든 멈출 수 있다. 자동 진행 턴도 일반 실행과 같은 경로를 타므로 Claude 세션에는 **토큰 한도 자동 전환과 `waiting_limit` 대기·자동 재개가 그대로 적용**된다(Codex·agy는 각자 구독 인증을 따른다). 목표가 걸린 세션은 `/status`에 `목표(자동 진행)` 줄로 표시된다. (목표 진행은 사용량을 빠르게 소모할 수 있으니 `/usage`로 한도를 함께 확인하는 것이 좋다.)
...
세 제공자 모두 실행 중 답변 본문이 자라는 모습을 `[RUNNING]` 상태 메시지에 끝부분 미리보기로 갱신한다(레이트리밋 때문에 디바운스 후 갱신, 끝부분 약 1200자만 표시). Codex는 도구·파일 변경 등 단계별 진행 요약과 함께 답변 텍스트 델타(`item.updated`)를 흘려보내고, agy는 `--print`의 stdout이 토막으로 도착할 때마다 경고 배너를 제거한 본문을 흘려보낸다. 완료 시에는 셋 모두 완성본 전체를 정식 메시지로 보낸다(미리보기와 중복 전송하지 않음). agy는 구조적으로 단계별 도구 진행 이벤트가 없어, 진행 표시는 답변 본문 미리보기로만 이뤄진다.
...
- **커넥터(MCP)** — `~/.claude.json`의 `mcpServers`와 `~/.codex/config.toml`의 `[mcp_servers.*]`를 하나로 병합한다(이름이 겹치면 `~/.claude.json` 우선). 병합 결과는 세 제공자에 각자 맞는 방식으로 전달된다.
  - **Claude**: 병합 셋을 Agent SDK의 `mcpServers`로 넘긴다. `LONG_RUNNING_MCP_SERVERS`에 든 서버만 `alwaysLoad`로 즉시 노출되고, 나머지는 tool search로 **지연 로딩**되어 도구 스키마가 한꺼번에 컨텍스트를 채우지 않는다(쓸 때만 검색해 불러옴).
  - **Codex**: `codex` 바이너리가 `~/.codex/config.toml`을 네이티브로 읽으므로 커넥터·스킬·플러그인을 그대로 상속한다(별도 주입 없음).
  - **agy**: 실행 직전 병합 커넥터를 agy가 네이티브로 읽는 `~/.gemini/config/mcp_config.json`에 동기화한다(기존 사용자 정의 서버와 다른 최상위 키는 보존하고 내용이 바뀐 경우에만 기록). agy는 `--dangerously-skip-permissions`로 동작하므로 동기화된 커넥터는 텔레그램 승인 없이 사용된다.
- **스킬** — Claude는 `settingSources: ['user']`로 `~/.claude/skills`와 전역 `CLAUDE.md`를 발견하고 `skills: 'all'`로 전부 켠다. 스킬은 이름·한 줄 설명만 선로딩되는 점진적 공개라 컨텍스트 비용이 작다. Codex(`~/.codex/skills`)와 agy(`~/.gemini/config/skills`)는 각 바이너리가 네이티브로 읽는다.
- **플러그인** — Claude는 `settingSources: ['user']`를 통해 사용자 설정에 등록·활성화된 플러그인을 데스크톱/Claude Code와 동일하게 로드한다. Codex(`~/.codex/plugins`)·agy(`~/.gemini/config/plugins`)는 네이티브로 읽는다.
...
| `mcp-policy.ts` | MCP 서버 항목 파싱·타임아웃 적용·재시도 판정(장기 실행 서버만 `alwaysLoad`, 나머지는 지연 로딩). |
| `connectors.ts` | 데스크톱 커넥터 공유. `~/.claude.json` + `~/.codex/config.toml`(미니 TOML 파서) 병합, Claude SDK용 `mcpServers` 빌드, agy용 `~/.gemini/config/mcp_config.json` 동기화. |
