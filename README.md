...
| AI 선택·성향 조절 | `/model`(제공자 전환 + 모델 변경), `/thinking`(Claude 깊이 사고 on/off), `/power`(현재 AI 작업량·추론 강도), `/lean`(최소 구현), `/mode`(승인 엄격도) |
| 정리 | `/reset`(같은 토픽의 대화 문맥 초기화), `/compact`(대화 요약 압축), `/memory`(배운 점을 장기 기억에 저장) |
