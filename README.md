...
**agy (선택)** — `agy`(Antigravity CLI) 바이너리를 설치하고 자체 로그인을 마쳐야 한다. 봇은 `~/.local/bin/agy`를 먼저 찾고, 없으면 PATH의 `agy`를 쓴다. 다른 위치라면 `.env`의 `AGY_EXECUTABLE`로 지정한다. agy를 설치하지 않았다면 Claude/Codex만 쓰면 된다.
...
`/mode`는 세 제공자에서 같은 의도로 작동한다. Claude는 텔레그램 승인 브로커에 직접 연결되고, Codex는 `plan=read-only`, `default/acceptEdits=workspace-write`, `dontAsk/auto=danger-full-access` 샌드박스로 매핑된다. agy는 `plan/default/acceptEdits`에서 `--sandbox`를 사용하고, `dontAsk/auto`에서 전체 작업 모드로 실행한다. Codex·agy는 비대화식 CLI이므로 실행 중 개별 도구 승인 버튼 대신 공통 안전 지침과 샌드박스로 경계를 강제한다.
...
- **커넥터·MCP 도구** — `~/.claude.json`과 `~/.codex/config.toml`의 MCP 서버를 병합한다(같은 이름이면 `~/.claude.json`이 우선). Claude에는 SDK 설정으로 직접 전달하고, Codex는 자기 `config.toml`을 네이티브로 읽으며, agy에는 같은 서버 목록을 `~/.gemini/config/mcp_config.json`에 동기화한다. stdio 서버의 토큰·환경변수는 설정 파일에 복제하지 않고 권한 600의 `~/.claude/shared-resources/connectors.json`에 두며, 공통 실행 래퍼 `scripts/run-shared-mcp.mjs`가 실행 시점에 주입한다.
...
`scripts/`는 OAuth 셋업, LaunchAgent 설치·재시작, 그리고 공통 커넥터 stdio 서버를 권한 600 레지스트리에서 실행하는 래퍼(`run-shared-mcp.mjs`) 스크립트, `tests/`는 vitest 테스트, `launchd/`는 plist 템플릿, `data/`는 런타임 SQLite와 로그(git 제외)다.
