...
텔레그램 채팅으로 내 Mac에 있는 AI에게 일을 시키고, 그 결과를 휴대폰으로 받아 보는 **개인용 AI 비서 봇**입니다. 텔레그램 그룹의 '주제(Topic)' 하나가 AI와의 대화 하나에 연결되며, **Claude · Codex · agy** 세 가지 AI 중 원하는 것을 골라 쓰고 대화 도중에 바꿀 수도 있습니다.

> 이 문서는 두 부분으로 나뉩니다.
> - **1부 — 사용 안내(누구나):** 봇이 무엇이고 텔레그램에서 어떻게 쓰는지. 비개발자도 이 부분만 읽으면 됩니다.
> - **2부 — 설치·운영(직접 띄우려는 분):** 봇을 자기 Mac에 설치해 돌리려는 분을 위한 기술 안내입니다.
...
평소 컴퓨터 앞에서 하던 코딩·문서 작성·자료 정리 같은 작업을, **외출 중에도 텔레그램 메시지 한 줄로 시작하고, 진행 상황을 지켜보고, 승인**할 수 있게 해 주는 봇입니다. 봇은 **본인 계정·본인 그룹에서 온 메시지만** 처리하므로 다른 사람은 쓸 수 없습니다.

## 무엇을 할 수 있나요?

- **작업 지시** — "이 프로젝트에 로그인 기능 추가해줘"처럼 적으면, AI가 미리 등록해 둔 폴더의 코드를 직접 읽고 고칩니다.
- **AI 선택·전환** — 새 작업을 시작할 때 **Claude / Codex / agy** 중 누구와 일할지 고르고, 작업 도중에도 `/model`로 바꿀 수 있습니다. 바꿔도 지금까지의 **대화 요약이 다음 AI에게 인계**되어 대화가 끊기지 않습니다.
- **중간 개입** — 작업 도중 방향을 바꾸거나(`/steer`), 끝난 뒤 할 일을 미리 예약(`/next`)하거나, **조건이 충족될 때까지 자동으로 반복**(`/goal`)하게 할 수 있습니다.
- **파일 주고받기** — 사진·문서·음성 등을 보내면 봇이 저장하고 그 위치를 AI에게 알려 주어 AI가 파일을 열어 봅니다. 반대로 결과 파일은 `/upload 경로`로 휴대폰에 받아볼 수 있습니다.
- **안전장치** — 위험할 수 있는 작업은 AI가 멋대로 진행하지 않고, **휴대폰으로 "이거 해도 될까요?" 하고 버튼으로 승인**을 받습니다.
- **사용량 확인** — 구독 한도가 얼마나 남았는지 `/usage`로 볼 수 있습니다.

## 기본 사용 흐름

1. 텔레그램 그룹에서 `/new`를 보내고, 작업할 **프로젝트(폴더)** 를 버튼으로 고릅니다.
2. 화면 아래 **기본값 패널**에서 어떤 AI(Claude/Codex/agy)·모델·사고 강도로 일할지 확인하거나 버튼으로 바꿉니다.
3. 하고 싶은 일을 **평소 말하듯** 적어 보냅니다. 그러면 새 주제(Topic)가 생기고 작업이 시작됩니다.
4. AI가 일하는 과정을 메시지로 중계하고, 승인이 필요하면 버튼으로 물어봅니다.
5. 작업이 끝나면 결과를 알려 줍니다. 같은 주제에 메시지를 더 보내면 **같은 대화를 이어 갑니다.**
...
| 사고(thinking) · 작업량(power/effort) | AI가 얼마나 깊게 고민할지 정하는 다이얼. 높일수록 신중하지만 사용량을 더 씁니다. |
...
| 하고 싶은 것 | 명령어 |
| --- | --- |
| 새 작업 시작 / 이어 말하기 | `/new` (작업이 끝난 뒤 같은 주제에 그냥 메시지를 보내면 대화 계속) |
| 진행 중 작업 다루기 | `/steer`(방향 바꾸기), `/next`(끝난 뒤 할 일 예약), `/goal`(조건 충족까지 자동 반복), `/stop`(중단), `/fork`(현재 대화 복제) |
| 파일 | 사진·문서·음성 등을 그냥 전송(봇이 저장 후 AI에게 위치 전달), `/upload 경로`(결과 파일을 휴대폰으로 받기) |
| 상태·사용량 확인 | `/status`(작업 상태), `/usage`(남은 한도), `/sessions`(최근 작업), `/projects`(등록 폴더), `/diff`(바뀐 코드 요약) |
| AI 선택·성향 조절 | `/model`(제공자 전환 + 모델 변경), `/thinking`(Claude 깊이 사고 on/off), `/power`(Claude 작업량), `/effort`(Codex 작업량), `/lean`(최소 구현), `/mode`(승인 엄격도) |
| 정리 | `/compact`(대화 요약 압축), `/memory`(배운 점을 장기 기억에 저장) |
| 관리 | `/addp`·`/deltp`(작업 폴더 추가·삭제), `/delete`(이 주제와 기록 삭제), `/doctor`(환경 점검) |

여기까지가 텔레그램으로 봇을 **쓰기만** 할 때 알아야 할 전부입니다. 아래 2부는 봇을 자기 Mac에 직접 설치해 돌리려는 분을 위한 기술 안내입니다.

---

# 2부. 설치·운영 (직접 띄우려는 분)

이 봇은 내 Mac에 상주하며, 텔레그램 Forum Topic 하나를 AI 세션 하나에 연결한다. 한 토픽에서 **Claude(Agent SDK)**, **Codex(Codex SDK)**, **agy(Antigravity CLI)** 중 하나로 대화하며, 같은 토픽 안에서 제공자를 바꿔도 직전 맥락을 요약으로 인계해 이어 간다.
...
- 명령: `/new`, `/addp`, `/deltp`, `/steer`, `/next`, `/goal`, `/fork`, `/stop`, `/compact`, `/memory`, `/mode`, `/model`, `/thinking`, `/power`, `/effort`, `/lean`, `/status`, `/sessions`, `/usage`, `/projects`, `/diff`, `/upload`, `/delete`, `/doctor`
...
**agy (선택)** — `agy`(Antigravity CLI) 바이너리를 설치하고 자체 로그인을 마쳐야 한다. 봇은 `~/.local/bin/agy`를 먼저 찾고, 없으면 PATH의 `agy`를 쓴다. 다른 위치라면 `.env`의 `AGY_EXECUTABLE`로 지정한다. agy 세션은 `--dangerously-skip-permissions`로 실행되므로 텔레그램 도구 승인을 거치지 않는다는 점에 유의한다. agy를 설치하지 않았다면 Claude/Codex만 쓰면 된다.

별도 Anthropic/OpenAI API 과금은 사용하지 않는다(모두 구독 인증).
...
새 세션의 제공자는 상시 기본값 패널(텔레그램 하단 reply 키보드)에서 고른다. 패널은 `🤖 제공자`(Claude → Codex → agy 순환), `🧠 모델`(현재 제공자의 모델 선택), `💭`(Claude면 thinking on/off, Codex면 추론 강도 순환, agy는 강도가 모델명에 내장되어 `🧠`로 변경)로 구성되며, 여기서 정한 값이 **다음에 만드는 새 세션의 기본값**으로 저장된다. `/new` 직후 이 패널과 함께 현재 기본값 요약이 표시된다.
...
- **agy**: 기본 `Gemini 3.1 Pro (High)`. 추론 강도가 모델명에 포함되어 있어 모델 선택으로 함께 정한다.

모델·thinking·추론 선택지는 시작 시 제공자 카탈로그(Claude=SDK `supportedModels`, Codex=번들 바이너리 `debug models`, agy=`agy models`)를 동적으로 읽어 채운다. 조회에 실패하면 정적 fallback 목록을 쓴다.
...
- `/power` — 작업량(effort, "더 빠름 ↔ 더 스마트함"). `low`, `medium`, `high`(기본), `xhigh`, `max`. 높을수록 깊게 사고하고 토큰을 더 쓴다.

각각 인자 없이 부르면 현재 값과 버튼을, 인자를 주면(`/thinking off`, `/power high`) 다음 실행부터 적용한다. 실행 중에는 바꿀 수 없고, `/status`에 `thinking`과 `Claude 작업량`으로 표시된다.

Codex의 reasoning effort는 기본 `high`이고 `/effort`로 세션별 변경한다(`minimal`, `low`, `medium`, `high`, `xhigh`). `/effort`는 Codex 전용이며 Claude 작업량은 `/power`가, agy는 모델명에 강도가 내장되어 별도 다이얼이 없다.
...
`/status`는 명령에 응답하는 것으로 오케스트레이터 프로세스가 살아 있음을 확인하고, 세션 토픽 안에서는 해당 작업이 실제로 현재 프로세스에서 실행 중인지 표시한다. 현재 제공자·모델·thinking·작업량·모드·목표 상태를 함께 보여 준다. 실행 중 진행 메시지는 새 도구 호출이 없어도 30초마다 경과시간을 갱신한다.
...
- **agy**: 단계별 도구 이벤트가 구조적으로 없어, `--print`의 stdout이 토막으로 도착할 때마다 경고 배너를 제거한 본문을 진행 표시로 흘려보낸다.
...
Claude Code는 컨텍스트 한도에 가까워지면 자동 압축한다. 토픽 작업이 끝난 상태에서 `/compact`를 실행하면 즉시 수동 압축하며, `/compact 인증 변경과 남은 테스트 중심`처럼 보존 초점을 지정할 수 있다.
...
- `/delete`를 실행하고 확인하면 Telegram 토픽, SQLite 세션·승인 기록, 로컬 Claude 대화 원본을 함께 삭제하고 실행·대기 작업도 취소한다.
...
세 제공자가 데스크톱 앱과 같은 커넥터(MCP)·스킬·플러그인을 쓰도록 배선되어 있다(`src/connectors.ts`).

- **커넥터(MCP)** — `~/.claude.json`의 `mcpServers`와 `~/.codex/config.toml`의 `[mcp_servers.*]`를 하나로 병합한다(이름이 겹치면 `~/.claude.json` 우선). 병합 결과는 세 제공자에 각자 맞는 방식으로 전달된다.
  - **Claude**: 병합 셋을 Agent SDK의 `mcpServers`로 넘긴다. `LONG_RUNNING_MCP_SERVERS`의 서버만 `alwaysLoad`로 즉시 노출되고, 나머지는 tool search로 **지연 로딩**되어 도구 스키마가 한꺼번에 컨텍스트를 채우지 않는다.
  - **Codex**: `codex` 바이너리가 `~/.codex/config.toml`을 네이티브로 읽으므로 그대로 상속한다(별도 주입 없음).
  - **agy**: 실행 직전 병합 커넥터를 agy가 네이티브로 읽는 `~/.gemini/config/mcp_config.json`에 동기화한다(기존 사용자 정의 서버·다른 최상위 키는 보존하고 내용이 바뀐 경우에만 기록). agy는 `--dangerously-skip-permissions`로 동작하므로 동기화된 커넥터는 텔레그램 승인 없이 사용된다.
- **스킬** — Claude는 `settingSources: ['user']`로 `~/.claude/skills`와 전역 `CLAUDE.md`를 발견하고 `skills: 'all'`로 전부 켠다(이름·한 줄 설명만 선로딩하는 점진적 공개라 컨텍스트 비용이 작다). Codex(`~/.codex/skills`)와 agy(`~/.gemini/config/skills`)는 각 바이너리가 네이티브로 읽는다.
- **플러그인** — Claude는 `settingSources: ['user']`로 사용자 설정에 등록·활성화된 플러그인을 데스크톱/Claude Code와 동일하게 로드한다. Codex(`~/.codex/plugins`)·agy(`~/.gemini/config/plugins`)는 네이티브로 읽는다.

`settingSources: ['user']`는 `~/.claude/settings.json`(스킬·플러그인·전역 지침)만 읽고 사전 승인 권한 규칙이 든 `settings.local.json`(`local` 소스)이나 프로젝트 설정은 읽지 않으므로, Claude의 모든 도구는 변함없이 `/mode`와 텔레그램 승인 브로커(`canUseTool`)를 거친다. 커넥터 소스 경로는 `CODEX_HOME`을 따르며 별도 `.env` 설정이 필요 없다.
...
- `/model`로 제공자·모델을, `/thinking`·`/power`로 Claude 사고·작업량을, `/effort`로 Codex 추론 작업량을, `/lean`으로 최소 구현 원칙을 세션별로 확인·변경한다(실행 중에는 변경 불가).
...
- agy는 `--dangerously-skip-permissions`로 동작하므로 agy 세션의 도구는 텔레그램 승인을 거치지 않는다(신뢰하는 작업에만 사용 권장).
...
- Telegram 봇은 [grammy](https://grammy.dev), Claude 실행은 `@anthropic-ai/claude-agent-sdk`, Codex 실행은 `@openai/codex-sdk`, agy는 `agy` CLI를 직접 spawn.
...
| `config.ts` | `.env`·`projects.json`을 읽어 `AppConfig` 생성. `/addp`·`/deltp` 프로젝트 추가·삭제, `agy`/`claude` 바이너리 경로 해석. |
...
| `store.ts` | SQLite 스키마·마이그레이션·CRUD. 컬럼 추가 시 기존 DB를 위해 `ALTER TABLE` 마이그레이션을 더한다. (`plan_*` 테이블은 과거 `/plan` 파이프라인 잔존 스키마로 현재 미사용.) |
...
| `connectors.ts` | 데스크톱 커넥터 공유. `~/.claude.json` + `~/.codex/config.toml`(미니 TOML 파서) 병합, Claude용 `mcpServers` 빌드, agy용 `~/.gemini/config/mcp_config.json` 동기화. |
...
4. `SessionManager`가 세션의 제공자에 따라 Claude Agent SDK / Codex SDK / agy CLI를 실행하고, 중간 출력은 `stream-renderer`·`telegram-transport`로 토픽에 보낸다.
