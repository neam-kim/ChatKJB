#!/usr/bin/env node
// mirror-to-cloudstorage.mjs를 하루 여러 번 실행하는 LaunchAgent를 설치한다(선택적 개인 백업).
// 다른 보조 에이전트와 같은 규약: node 경로=process.execPath, 로그는 CloudStorage 밖.
// CHATKJB_LOG_ROOT가 있으면 해당 상위 경로를 사용한다.
//
// 사전 조건: .env 또는 셸에 CHATKJB_MIRROR_DEST 설정
// 사용: node scripts/install-mirror-agent.mjs
//       node scripts/install-mirror-agent.mjs --hours 1,7,13,19
...
const label = "com.chatkjb.cloudstorage-mirror";
const projectDir = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const nodePath = process.execPath;
const scriptPath = join(projectDir, "scripts", "mirror-to-cloudstorage.mjs");
...
const hours = argstr("--hours", "1,7,13,19")
  .split(",")
  .map((h) => Number(h.trim()))
...
const mirrorDest = (process.env.CHATKJB_MIRROR_DEST || dotenv.CHATKJB_MIRROR_DEST || "").trim();
if (!mirrorDest) {
  console.error(
    "CHATKJB_MIRROR_DEST 가 필요합니다. .env에 미러 대상 절대 경로를 넣거나 셸에서 export 하십시오."
  );
  console.error("CloudStorage 미러는 선택적 개인 백업 기능이며 일반 설치에는 필요하지 않습니다.");
  process.exit(1);
}
...
<dict>
  ...
  <dict>
    ...
    <key>CHATKJB_MIRROR_DEST</key>
    <string>${xml(mirrorDest)}</string>
  </dict>
...
console.log(`CloudStorage-mirror LaunchAgent installed: ${agentPath}`);
console.log(`  dest: ${mirrorDest}`);
