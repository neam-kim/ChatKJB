...
//   2) global-project-results*.md — .result.md 병합본 snapshots
...
function collectResultSnapshotFiles(directories = [INBOX_DIR, RAW_DIR]) {
  const files = [];
  const pattern = /^global-project-results(?:-\d{4}-\d{2}-\d{2}-\d{2}-\d{2})?\.md$/;
  const walk = (dir) => {
    if (!existsSync(dir)) return;
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      const path = join(dir, entry.name);
...
function collectResultEntries(directories = [INBOX_DIR, RAW_DIR]) {
  const seen = new Set();
  const entries = [];
  for (const file of collectResultSnapshotFiles(directories)) {
    let text;
    try {
...
function main() {
  ...
  if (!resultEntries.length) log("결과 로그 snapshot 없음:", INBOX_DIR, RAW_DIR);
