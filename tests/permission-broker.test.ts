...
describe("PermissionBroker", () => {
  ...
  it("returns the same answers through the provider-neutral adapter", async () => {
    const { session, transport, broker } = setup();
    const controller = new AbortController();
    const decision = broker.requestUserInput(session, {
      questions: [{
        question: "설치 범위는?",
        options: [{ label: "로컬" }, { label: "전역" }]
      }]
    }, controller.signal);
    await new Promise((resolve) => setTimeout(resolve, 0));

    await broker.handleCallback(callbackData(transport, "로컬"));

    await expect(decision).resolves.toEqual({ "설치 범위는?": "로컬" });
    expect(transport.messages.map((message) => message.text)).toContain(
      "[PROGRESS] 선택을 받았습니다. 작업을 계속합니다."
    );
  });

  it("rejects a second pending question for the same session without replacing the first", async () => {
    const { session, transport, broker } = setup();
    const controller = new AbortController();
    const first = broker.requestUserInput(session, {
      questions: [{
        question: "첫 질문은?",
        options: [{ label: "A" }, { label: "B" }]
      }]
    ...
    await expect(broker.requestUserInput(session, {
      questions: [{
        question: "두 번째 질문은?",
        options: [{ label: "C" }, { label: "D" }]
      }]
    }, controller.signal)).rejects.toThrow("이미 응답을 기다리는 질문");

    await broker.handleCallback(callbackData(transport, "A"));
    await expect(first).resolves.toEqual({ "첫 질문은?": "A" });
  });
