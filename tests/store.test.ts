...
describe("StateStore", () => {
  ...
  it("marks unfinished work interrupted on restart", () => {
    ...
    expect(interrupted[0]?.id).toBe(session.id);
  ...
  it("persists grokSessionId for multi-turn resume and defaults new sessions to null", () => {
    const store = makeStore();
    const session = makeSession(store.db.name.replace(/\/state\.sqlite$/, ""));
    store.createSession(session);

    // 새 세션은 grok 세션이 아직 없으므로 null(첫 grok 턴에서 --session-id 생성).
    expect(store.getSession(session.id)?.grokSessionId ?? null).toBeNull();
    // 첫 턴 성공 후 grok 세션 UUID를 저장하면 다음 턴이 --resume 한다.
    store.updateSession(session.id, { grokSessionId: "11111111-2222-3333-4444-555555555555" });
    expect(store.getSession(session.id)?.grokSessionId).toBe("11111111-2222-3333-4444-555555555555");
    // 제공사 전환·문맥 초기화 시 마커를 비우면 다음 grok 턴이 새 세션을 만든다.
    store.updateSession(session.id, { grokSessionId: null });
    expect(store.getSession(session.id)?.grokSessionId ?? null).toBeNull();
    store.close();
  });
  ...
  it("persists and normalizes the new-session agy thinking default", () => {
    ...
    expect(store.getSessionDefaults().agyThinkingLevel).toBe("");
    expect(store.updateSessionDefaults({ agyThinkingLevel: "high" }).agyThinkingLevel)
      .toBe("high");
    expect(store.updateSessionDefaults({ agyThinkingLevel: "minimal" }).agyThinkingLevel)
      .toBe("");
    store.db.prepare(
