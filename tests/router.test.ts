...
import { wikiVaultCandidates } from "../src/wiki-paths.js";
...
describe("provider 라우팅", () => {
  ...
  it("Grok만 인증된 설치에서도 Grok으로 폴백한다", () => {
    expect(routeProvider("버그 수정", {}, ["grok"]).provider).toBe("grok");
  });
});
...
describe("LLM-Wiki 경로", () => {
  ...
  it("WIKI_VAULT가 없으면 마운트된 볼륨의 위키를 우선 감지하고 없으면 저장소 형제로 폴백한다", () => {
    const preferredWiki = wikiVaultCandidates(homedir()).find((candidate) => existsSync(candidate));
    const repoSiblingWiki = join(dirname(process.cwd()), "LLM-Wiki");
    expect(wikiVaultPath({} as NodeJS.ProcessEnv)).toBe(
      preferredWiki ?? repoSiblingWiki
    );
  });
