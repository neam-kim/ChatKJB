...
describe("parseCodexAccountHomes", () => {
  it("returns no Codex accounts when the implicit fallback is not authenticated", () => {
    const result = parseCodexAccountHomes(undefined, "/tmp/fallback-home");
    expect(result).toEqual([]);
  });

  it("uses an authenticated implicit fallback home", () => {
    const home = makeHome();
    expect(parseCodexAccountHomes("", home)).toEqual([home]);
  });
