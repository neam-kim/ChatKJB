...
function defaultWikiVaultPath(env: NodeJS.ProcessEnv = process.env): string {
  const configured = env.LLM_WIKI_ROOT || env.WIKI_VAULT;
  if (configured) return configured;
  const home = homedir();
  // 어느 후보도 실제로 없으면 README가 안내하는 규약(저장소와 나란히 둔 LLM-Wiki)을
  // 기본값으로 삼는다. 마운트 상태에 따라 기본값이 달라지지 않도록 고정한다.
  const repoSibling = resolve(dirname(projectSourceDir()), "LLM-Wiki");
  const candidates = [
  ...
  return candidates.find((candidate) => existsSync(candidate)) ?? repoSibling;
}

// LLM-Wiki 저장소 루트. 환경변수 LLM_WIKI_ROOT/WIKI_VAULT로 위치를 바꾼 경우도 따른다.
export function wikiVaultPath(env: NodeJS.ProcessEnv = process.env): string {
  return filesystemPath(defaultWikiVaultPath(env));
}
