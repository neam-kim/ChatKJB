...
import { appTimeZone } from "./localization.js";
...
// 시간대가 생략되면 배포 호스트의 설정(TZ 또는 시스템 시간대)을 사용한다.
...
// Codex 스트림이 서버 측에서 조기 종료돼(웹소켓이 response.completed 전에 닫힘) 턴이
// 실패했는지 판별한다. SDK가 자체적으로 몇 회 재연결(Reconnecting... N/5)한 뒤에도
// 회복하지 못하면 이 문구로 error 이벤트가 방출된다. 토큰 한도가 아니라 일시적 네트워크/
// 세션 끊김이므로, 계정을 봉인/전환하지 말고 같은 스레드로 짧은 백오프 후 재시도하면
// 대부분 회복된다. rate-limit 문구는 isRateLimitError가 우선 처리하므로 여기서 겹치지 않는다.
export function isTransientStreamError(error: unknown): boolean {
  const message = (error instanceof Error ? error.message : String(error)).toLowerCase();
  return (
    message.includes("websocket closed")
    || message.includes("stream disconnected")
    || message.includes("disconnected before completion")
    || message.includes("before response.completed")
    || message.includes("reconnecting")
    || (message.includes("stream") && message.includes("closed by server"))
  );
}
