...
export function buildSharedPolicyText(paths: SharedMemoryTextPaths & { home: string; }): string {
  return [
    ...
    "",
    "After every conversation, append at least one dated sentence to `.result.md` in each project folder summarizing the request, suggestion, decision, approval or rejection, and result. This applies to read-only, advisory, aborted, and trivial conversations too.",
    "Create `.result.md` if absent. The only acceptable omission is a genuine inability to write to the project folder, which must be reported explicitly.",
    "",
